  ...
  module FileUtils
    ...
    def ln(source, dest, options = {})
      ...
      unless options[:force]
        fail Errno::EPERM, "#{source} or #{dest}" if File.directory?(source)
        fail Errno::ENOENT, "#{source} or #{dest}" unless File.exists?(source)
        fail Errno::EEXIST, "#{source} or #{dest}" if File.exists?(dest)
      end
